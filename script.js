document.addEventListener('DOMContentLoaded', () => {
    ...
    whatsappCheckoutBtn.addEventListener('click', () => {
        ...
        whatsappMessage += `Subtotal: ₹${(totals.subtotal + totals.currentDiscount).toFixed(2)}%0A`; // Show subtotal before discount
        if (couponApplied && totals.currentDiscount > 0) { // Only show discount if applied and greater than 0
            whatsappMessage += `Discount (${(9.5).toFixed(1)}%): -₹${totals.currentDiscount.toFixed(2)}%0A`;
        }
