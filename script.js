document.addEventListener('DOMContentLoaded', () => {
    ...
    const calculateCartTotals = () => {
        ...
        // Delivery charge logic (if any specific logic is needed, otherwise 0)
        // For example: if (subtotal < 200) deliveryCharge = 30;

        const gst = subtotal * 0.05; // 5% GST
        const total = subtotal + gst + codCharge + deliveryCharge; // Add COD charge to total
    ...
    const updateCartUI = () => {
        ...
        document.querySelectorAll('.increase-item-qty').forEach(button => {
            button.addEventListener('click', (event) => {
                event.stopPropagation(); // Prevent event from bubbling up and closing cart
                const productId = event.currentTarget.dataset.productId; // Use currentTarget
                updateCartItemQuantity(productId, 1);
            });
        });

        document.querySelectorAll('.decrease-item-qty').forEach(button => {
            button.addEventListener('click', (event) => {
                event.stopPropagation(); // Prevent event from bubbling up and closing cart
                const productId = event.currentTarget.dataset.productId; // Use currentTarget
                updateCartItemQuantity(productId, -1);
        ...
        // Update delivery info message based on COD selection
        const selectedPaymentMethod = document.querySelector('input[name="paymentMethod"]:checked')?.value;
        if (selectedPaymentMethod === 'Cash On Delivery') {
            deliveryInfoMessage.textContent = 'Delivery charges apply for COD orders below ₹500. GST applies to all orders.';
        } else {
            deliveryInfoMessage.textContent = 'GST applies to all orders.';
        }
    ...
    const showProductAddedToast = () => {
        ...
        productAddedToast.querySelector('.toast-body').textContent = 'Product added to cart successfully!';
